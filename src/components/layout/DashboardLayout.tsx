...
export function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="min-h-screen flex bg-background text-foreground">
      {/* Sidebar */}
      <aside
        className={`bg-card border-r border-border flex flex-col transition-all duration-normal ease-standard ${
          collapsed ? 'w-sidebar-collapsed' : 'w-sidebar'
        }`}
      >
        {/* Header */}
        <div className="h-header border-b border-border flex items-center justify-between px-4 shrink-0">
          {!collapsed && (
            <span className="flex items-center gap-2 text-body-sm font-semibold tracking-tight text-foreground">
              <span className="h-6 w-6 rounded-md bg-accent-blue text-white grid place-items-center text-[11px] font-bold">I</span>
              ISOCODELABS
            </span>
          )}
          <button
            ...
            className="p-1.5 rounded-md hover:bg-apple-gray dark:hover:bg-sf-bg-elevatedDark text-[var(--text-secondary)] ml-auto"
          >
        ...
        {/* Navigation */}
        <nav className="flex-1 py-4 px-2 space-y-1 overflow-y-auto no-scrollbar" aria-label="Primary">
          {navItems.map((item) => {
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-md text-body-sm font-medium transition-colors duration-fast ease-standard focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-blue ${
                  isActive
                    ? 'bg-accent-blue text-white'
                    : 'text-[var(--text-secondary)] hover:bg-apple-gray dark:hover:bg-sf-bg-elevatedDark hover:text-foreground'
                }`}
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {/* TopBar */}
        <header className="h-header border-b border-border bg-card flex items-center justify-between px-6 z-sticky shrink-0">
          <div className="flex items-center gap-4 w-96">
            <div className="relative w-full">
              ...
              <input
                ...
                aria-label="Search"
                className="w-full pl-9 pr-4 py-2 text-body-sm bg-apple-gray dark:bg-sf-bg-elevatedDark border border-border rounded-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-accent-blue focus-visible:border-accent-blue"
              />
            </div>
          </div>
        ...
        {/* Content Area */}
        <main className="flex-1 overflow-y-auto overflow-x-hidden p-8 no-scrollbar bg-background">
